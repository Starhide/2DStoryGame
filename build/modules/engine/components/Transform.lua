...
function Transform.new(entity, init)
    ...
    o.entity = entity
    o.position = Vector(init.position or {x = 0, y = 0})
    o.scale = Vector(init.scale or {x = 1, y = 1})
    o.origin = Vector(init.origin or {x = 0, y = 0})
...
function Transform:getPosition()
    if self.entity.parent then
		return self.entity.parent.Transform:getPosition() + self.position
	end
...
function Transform:getScale()
    if self.entity.parent then
		return self.entity.parent.Transform:getScale() * self.scale
	end
    return self.scale
end
...
function Transform:getRotation()
    if self.entity.parent then
        return self.entity.parent.Transform:getRotation() + self.rotation
	end
...
function Transform:rotateTowards(x, y)
	if x - self.position.x < 0 then
		self.rotation = math.deg(math.atan((y - self.position.y)/(x - self.position.x)))-90
		else
		self.rotation = math.deg(math.atan((y - self.position.y)/(x - self.position.x)))+90
	end
	self.needsUpdate = true
